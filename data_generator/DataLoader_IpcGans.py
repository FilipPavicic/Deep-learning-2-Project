# -*- coding: utf-8 -*-
"""
Created on Mon Dec 26 15:06:51 2022
...
class CACD(data.Dataset):
    def __init__(
        ...
        label_pair_root = list_root + "/train_label_pair.txt"
        with open(label_pair_root, 'r', errors='replace') as f:
            lines = f.readlines()
        ...
        # define group_images
        group_lists = [
            list_root + '/train_age_group_0.txt',
            list_root + '/train_age_group_1.txt',
            list_root + '/train_age_group_2.txt',
            list_root + '/train_age_group_3.txt',
            list_root + '/train_age_group_4.txt'
        ]

        self.label_group_images = []
        for i in range(len(group_lists)):
            with open(group_lists[i], 'r', errors='replace') as f:
                lines = f.readlines()
                lines = [line.strip() for line in lines]
            group_images = []
            for l in lines:
                items = l.split()
                group_images.append(data_root + '/' + items[0])
            self.label_group_images.append(group_images)

        # define train.txt
        if self.split == "train":
            self.source_images = []  # which use to aging transfer
            with open(list_root + '/train.txt', 'r', errors='replace') as f:
                lines = f.readlines()
                lines = [line.strip() for line in lines]
            shuffle(lines)
            for l in lines:
                items = l.split()
                self.source_images.append(data_root + '/' + items[0])
        else:
            self.source_images = []  # which use to aging transfer
            with open(list_root + '/test.txt', 'r', errors='replace') as f:
                lines = f.readlines()
                lines = [line.strip() for line in lines]
            shuffle(lines)
            for l in lines:
...
if __name__ == "__main__":
    ...
    CACD_dataset = CACD(transforms=transforms, label_transforms=label_transforms)
